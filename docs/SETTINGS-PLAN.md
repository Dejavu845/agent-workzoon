...
**禁止**：Rust / 改名 / `.keel/` / 拆仓 / 重写 preview / `motion/react` / policy 放 catalog / wiki 义务做成布尔 / 破坏性动作对 Agent 开 allow / 引擎 spawn Agent / 在 `/workspace` 主树建 worktree。

---

## F. 实施清单（按阶段，每阶段一 commit）

分支：`cursor/workzoon-wiki-forest-b80f`。每阶段：`pnpm test` 绿 → commit → push。
...
- 不在 `/workspace` 主树 `git worktree add`。
