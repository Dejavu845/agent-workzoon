...
3. 不重做已勾。不发明第四视图。不引 Three.js / GSAP / `motion/react`。不在 `/workspace` 上 `git worktree add`。不提交根 `pnpm-lock.yaml`。
4. 测试绿 → commit → push → 更新 PR（base `cursor/workzoon-harbor-loop-b80f`）。
