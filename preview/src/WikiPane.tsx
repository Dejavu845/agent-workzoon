...
export function WikiPane({
  ...
  return (
    <div className="pane wiki-pane">
      <h2>百科</h2>
      <p className="hint">这里只收原文。谁能写进百科，由你定。`/` 搜索，j/k 换页。</p>
      ...
      <p className="hint">
        {searching
          ? visible.length
            ? `命中 ${visible.length} · wiki ${wikiCount} · raw ${rawCount}`
            : `没有命中 “${query.trim()}”`
          : `${wikiCount} wiki · ${rawCount} raw`}
      </p>
      {visible.length ? (
          ...
          {visible.map((item) => (
            <li key={item.path} className={page?.path === item.path ? "on" : ""}>
              <button type="button" className="wiki-hit" onClick={() => onOpen(item.path)}>
                ...
                <small>
                  {item.layer} · {item.type}
                  {item.compiled ? " · 引擎" : ""} · {item.path}
                </small>
              </button>
              {page?.path === item.path ? (
        ...
        <p className="empty">换个词，或清空后从 index 再走。</p>
      ) : null}
      <h3>收入原文</h3>
      <input value={ingestTitle} onChange={(event) => onTitle(event.target.value)} placeholder="来源标题" />
      <textarea value={ingestBody} onChange={(event) => onBody(event.target.value)} placeholder="原文。写入 raw/，不改 wiki。" />
      <div className="row">
