...
const spring = { type: "spring" as const, stiffness: 520, damping: 36, mass: 0.72 };
...
export function popIn(el: Element | null): void {
  ...
  animateMini(el as HTMLElement, { opacity: [0, 1], transform: ["scale(0.98)", "none"] }, { duration: 0.2, ease: [0.22, 1, 0.36, 1] });
}
...
export function fadeScene(el: Element | null, dir: -1 | 0 | 1): void {
  ...
  animateMini(
    el as HTMLElement,
    { opacity: [0, 1], transform: [`translateX(${x}px)`, "none"] },
...
export function receivePane(el: Element | null): void {
  ...
  animateMini(
    el.querySelector(".pane-shell") || el,
    { opacity: [0.7, 1], transform: ["translateX(8px)", "none"] },
    { duration: 0.22, ease: [0.22, 1, 0.36, 1] },
  );
  window.setTimeout(() => el.classList.remove("is-receiving"), 380);
}
...
export function bindChrome(root: Element | null): () => void {
  ...
  const stopPress = press(pressable, (element) => {
    animate(element, { transform: "scale(0.985)" }, { duration: 0.1 });
    return () => animate(element, { transform: "none" }, spring);
  });
  return () => {
    stopPress();
  };
...
export function useGlassSpecular(host: RefObject<HTMLElement | null>, enabled: boolean): void {
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      tx = ((event.clientX - box.left) / box.width) * 100;
      ty = ((event.clientY - box.top) / box.height) * 100;
      requestPaint();
    };
