...
## Decisions
${input.decisions?.trim() || "- (fill before marking done)"}

## Live state
${input.liveState?.trim() || (files ? files : "- No recorded file changes yet.")}
...
## Next action
${input.nextAction?.trim() || "- Continue from the current worktree. Do not expand scope."}
...
export function resumePrompt(project: Project, task: Task): string {
  const body = readHandoff(project, task.id) || "(no handoff yet)";
  const frozen = task.escalation
    ? "Do not touch frozen layers unless the task explicitly allows it. Stay within that reason."
    : "Do not touch frozen layers unless the task explicitly allows it.";
  const worktree = project.worktrees.find((item) => item.id === task.worktreeId);
  const branch = worktree?.branch || plannedBranch(task);
  return [
    `You are taking over task "${task.title}" in ${project.name}.`,
    `Work only in this worktree. Follow .lattice/charter.md.`,
    `Read WORKZOON.md at the worktree root first; if it is not visible (cloud checkout), read .lattice/handoffs/${task.id}.md as tracked on branch ${branch}.`,
    `Allowed: ${task.allowedPaths.join(", ") || "non-frozen layers only"}.`,
    frozen,
    "",
    body,
  ].join("\n");
