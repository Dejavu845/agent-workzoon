...
import { occupancyForProject } from "./occupancy.ts";
...
function guard(
  ...
  try {
    ...
    if (error instanceof PolicyError && error.code === "ask" && caller.actor === "agent" && !caller.consent) {
      ...
      throw new PolicyError("ask", action, `${error.message} requestId=${request.id}`, request.id);
    }
    throw error;
  }
}
...
export function defaultCharter(name: string) {
  return {
    version: 1,
    purpose: `${name} is developed under a fixed framework. Agents follow layers and do not rewrite earlier work.`,
    architecture: "Keep the charter, isolated worktrees, and file-based handoffs as the control plane.",
    conventions: [
      "One task, one worktree, one branch <intent>/<slug> (feat, spike, or hotfix).",
      "Write .lattice/handoffs/<taskId>.md before marking done.",
      "Read .lattice/wiki/index.md first; never edit .lattice/raw after write.",
      "Do not invent music/album metadata or commit secrets.",
    ],
...
export type DoneRejectKind = "handoff" | "head" | "blast" | "wiki-raw" | "wiki-forbidden";

export function doneRejectReason(kind: DoneRejectKind, detail: { slug?: string; count?: number } = {}): string {
  ...
  if (kind === "wiki-raw") {
    return `百科还拦着。先把「${detail.slug || "这张卡"}」的交接原文写进百科（还有 ${detail.count ?? 0} 条没写）。`;
  }
...
export function markDone(rootPath: string, taskId: string, caller: Caller = AGENT): Project {
  ...
  if (policy.wiki.gate === "block" && uncompiled.length) {
    throw new Error(doneRejectReason("wiki-raw", { slug: task.slug, count: uncompiled.length }));
  }
