...
const DECISIONS_PLACEHOLDER = "(fill before marking done)";
const NEXT_PLACEHOLDER = "Continue from the current worktree";
...
const CELLS: { id: EvidenceCell["id"]; label: string }[] = [
  { id: "tree", label: "开树" },
  { id: "diff", label: "有 diff" },
  { id: "blast", label: "blast 未阻" },
...
export function handoffFilled(body: string): boolean {
  ...
  if (decisions.includes(DECISIONS_PLACEHOLDER)) return false;
  if (next.includes(NEXT_PLACEHOLDER)) return false;
...
export function occupancyForTask(
  ...
  const derived = deriveDoing({
    ...
    pending: request ? request.action : "",
